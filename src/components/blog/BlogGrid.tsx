  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {mockBlogPosts.map((post) => (
        <Card key={post.id} className="bg-white border-2 border-robin-orange/10 hover:border-robin-orange/30 transition-colors">
          <Link to={`/blog/${post.slug}`}>
            <div className="overflow-hidden rounded-md">
              <img
                src={post.image}
                alt={post.title}
                className="w-full h-48 object-cover rounded-md transition-transform transform-gpu hover:scale-105"
              />
            </div>
